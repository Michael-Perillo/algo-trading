...
#   timestamp: 2025-06-23T22:03:41+00:00
...
class AccountConfigurations(BaseModel):
    ...
    suspend_trade: Optional[bool] = Field(None, description='If true, new orders are blocked.')
    ...
    max_margin_multiplier: Optional[str] = Field(None, description='Can be "1", "2", or "4"')
...
class Clock(BaseModel):
    ...
    is_open: Optional[bool] = Field(None, description='Whether or not the market is open\n')
    next_open: Optional[datetime] = Field(None, description='Next Market open timestamp')
    next_close: Optional[datetime] = Field(None, description='Next market close timestamp')
...
class CanceledOrderResponse(BaseModel):
    ...
    status: Optional[int] = Field(None, description='http response code', examples=[200])
...
class WhitelistedAddress(BaseModel):
    ...
    chain: Optional[str] = Field(None, description='Underlying network this address represents')
...
class Account(BaseModel):
    ...
    pending_transfer_in: Optional[str] = Field(None, description='Cash pending transfer in.')
    pending_transfer_out: Optional[str] = Field(None, description='Cash pending transfer out.')
    ...
    equity: Optional[str] = Field(None, description='Cash + long_market_value + short_market_value')
...
class TradingActivities(BaseModel):
    ...
    order_id: Optional[UUID] = Field(None, description='The id for the order that filled.')
    type: Optional[Type] = Field(None, description='fill or partial_fill', examples=['fill'])
...
class OrderLeg(BaseModel):
    ...
    replaces: Optional[UUID] = Field(None, description='The order ID that this order replaces')
    ...
    filled_qty: Optional[constr(min_length=1)] = Field(None, description='Filled quantity')
    filled_avg_price: Optional[str] = Field(None, description='Filled average price')
    order_class: Optional[OrderClass] = None
    order_type: Optional[str] = Field(None, description='Deprecated in favour of the field "type" ')
    type: OrderType
    side: OrderSide
...
class CryptoTransfer(BaseModel):
    ...
    tx_hash: Optional[str] = Field(None, description='On-chain transaction hash (e.g. 0xabc...xyz)')
    ...
    usd_value: Optional[str] = Field(None, description='Equivalent USD value at time of transfer')
    network_fee: Optional[str] = None
    fees: Optional[str] = None
    chain: Optional[str] = Field(None, description='Underlying network for given transfer')
    asset: Optional[str] = Field(
        None, description='Symbol of crypto asset for given transfer (e.g. BTC )'
    )
    from_address: Optional[str] = Field(None, description='Originating address of the transfer')
    to_address: Optional[str] = Field(None, description='Destination address of the transfer')
    created_at: Optional[datetime] = Field(None, description='Timedate when transfer was created')
