...
#   timestamp: 2025-07-11T20:27:48+00:00
...
class BarRequest(BaseModel):
    ...
    start: date | None = Field(None, description='Start time for the bar data')
    end: date | None = Field(None, description='End time for the bar data')


class OrderResponse(BaseModel):
    id: str | None = Field(None, description='Order identifier')


class Allocation(BaseModel):
    quantity: float = Field(..., description='Quantity to allocate for the order.')
    entry_price: float = Field(..., description='Intended entry price for the allocation.')
    stop_loss_price: float = Field(..., description='Stop loss price for risk management.')
    take_profit_price: float | None = Field(
        None, description='Take profit price for the allocation.'
    )
    metadata: dict[str, Any] | None = Field(
        None, description='Arbitrary metadata for the allocation.'
    )
